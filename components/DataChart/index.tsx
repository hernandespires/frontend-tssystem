...
const sexChartConfig = {
	...
	male: { label: "Masculino", color: "#1490F5" },
	female: { label: "Feminino", color: "#FF5CDC" }
...
const chartConfigOperations = {
	quantity: { label: "Quantidade" },
	_01: { label: "01", color: "#FFA300" },
	_02: { label: "02", color: "#fffd6a" },
	_03: { label: "03", color: "#1490F5" }
} satisfies ChartConfig
...
const DashboardCharts = ({ datas }: { datas: Employee[] }) => {
	...
	useEffect(() => {
		...
		const result: SexChartItem[] = [
			{ sex: "male", quantity: male, fill: "#1490F5" },
			{ sex: "female", quantity: female, fill: "#FF5CDC" }
		]
	...
	return (
		<div className="flex rounded-xl items-center h-81.5">
			...
			<Card className="bg-transparent w-full border-default-border-color gap-0 rounded-none border-0 border-r shadow-none">
				...
				<CardContent className="flex justify-center">
					<ChartContainer
						config={chartConfigCivilState}
						className="aspect-square w-48"
					>
						<PieChart>
							<ChartTooltip content={<ChartTooltipContent />} />
							<Pie
								data={civilStateChartData}
								dataKey="quantity"
								nameKey="civilState"
							/>
						</PieChart>
					...
					>
						...
						>
							...
							<YAxis
								...
								tickMargin={10}
								tickFormatter={(value) =>
									chartConfigOperations[
										value as keyof typeof chartConfigOperations
									]?.label ?? value
								}
							/>
							<ChartTooltip content={<ChartTooltipContent />} />

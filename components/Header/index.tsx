import Image from "next/image"
import { Button } from "../ui/button"
import { Avatar, AvatarFallback, AvatarImage } from "../ui/avatar"
import { FaArrowDown } from "react-icons/fa"

const Header = ({ department }: { department: string }) => (
    <header className="glass-effect flex justify-between items-center px-40 py-4.5 border-1 border-default-border-color rounded-md">
        <section className="flex gap-5">
            <Image src="/LOGO-ts2.svg" width="180" height="55" alt="Logo" />
            <div className="flex-col text-xl">
                <p className="text-white">
                    Departamento:
                </p>
                <span className="font-bold text-default-orange uppercase">
                    { department }
                </span>
            </div>
        </section>
        <section className="flex gap-6">
            <Button size="icon-lg" className="w-16 h-15">
                <Image src="/line-md_bell-filled.svg" width="32" height="32" alt="Notificações" />
            </Button>
            <Button className="min-w-45 h-15 border-1 border-default-orange bg-transparent gap-2">
                <Avatar className="w-12 h-12">
                    <AvatarImage src="https://github.com/shadcn.png" alt="Avatar" />
                    <AvatarFallback>
                        TS
                    </AvatarFallback>
                </Avatar>
                <p className="font-bold text-default-orange text-xl">
                    Perfil
                </p>
                <FaArrowDown color="#ffa300" width="20" />
            </Button>
        </section>
    </header>
)

export default Header
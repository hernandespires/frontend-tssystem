...
const DataMetrics = ({ department, datas }: { department: string; datas: string[] }) => {
	...
	return (
		<section className="border border-default-border-color w-full rounded-md py-3 h-full">
			{datas?.length > 0 ? (
				<>
					<div className="flex justify-between px-5.5">
						<h1 className="section-title">Métricas dos {department}</h1>
				...
				<div className="flex justify-center items-center h-full flex-col gap-8">
					...
					<h1 className="font-bold text-gray-300">Adicione um novo funcionário para as obter as métricas</h1>
				</div>
			)}
		</section>
	)

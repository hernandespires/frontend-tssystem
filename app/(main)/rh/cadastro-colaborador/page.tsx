...
const CadastroColaborador = () => {
    ...
    return (
        <>
            {
                actualStep === 2 ? <PersonalInformation urlPath={path} prevStep={() => route.push("/rh")} nextStep={() => setActualStep(2)} actualStep={actualStep} percentageProgress={20} />
                : actualStep === 1 ? <LaborDocuments urlPath={path} prevStep={() => setActualStep(1)} actualStep={actualStep} percentageProgress={40} nextStep={() => setActualStep(3)} /> 
                : actualStep === 3 ? <BankDetails urlPath={path} prevStep={() => setActualStep(1)} actualStep={actualStep} percentageProgress={60} nextStep={() => setActualStep(4)} /> 

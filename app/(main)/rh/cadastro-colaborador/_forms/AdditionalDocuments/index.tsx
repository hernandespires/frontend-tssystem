...
const AdditionalDocuments = ({
  ...
  const form = useZodForm(formSchema, {
    defaultValues: {
      additionalDocuments:
        uploadData.additionalDocuments ??
        employeeData.additionalDocuments ??
        []
    }
  })

  // 🔥 Sincroniza UploadContext → RHF
  useEffect(() => {
    form.setValue(
      "additionalDocuments",
      uploadData.additionalDocuments ?? [],
  ...
  const firstErrorKey = useGetFirstErrorKey(
    errors,
      ...
      >
        ...
        <Field className="flex min-h-112 py-3">
          <div className="flex flex-col items-center w-fit">
            ...
            <FieldError>
              {
                firstErrorKey === "additionalDocuments" &&
                String(form.formState.errors.additionalDocuments?.message)
              }
            </FieldError>

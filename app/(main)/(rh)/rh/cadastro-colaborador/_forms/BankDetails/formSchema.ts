...
export const formSchema = z
	.object({
		...
		agency: z
			.string({ error: defaultError("Agência") })
			.min(4, defaultError("Agência"))
			.nonempty(defaultEmptyError("Agência")),
		account: z
			.string({ error: defaultError("Conta") })
			.min(6, defaultError("Conta"))
			.nonempty(defaultEmptyError("Conta")),

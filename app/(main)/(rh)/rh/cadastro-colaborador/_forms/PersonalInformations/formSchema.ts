...
export const formSchema = z.object({
	...
	birthday: z
		.any()
		.nonoptional(defaultEmptyError("Data de nascimento"))
		.transform((v) => new Date(v))
		.refine((d) => !isNaN(d.getTime()), "Data inválida"),
	sex: z.string({ error: defaultError("Sexo") }).nonoptional(defaultEmptyError("Sexo")),
	civilState: z.enum(["SINGLE", "DATING", "MARRIED", "WIDOWED"], { error: defaultError("Estado civil") }).nonoptional(defaultEmptyError("Estado civil")),
	// nacionality: z.enum(["BRAZILIAN", "AMERICAN"], { error: defaultError("Nacionalidade") }).nonoptional(defaultEmptyError("Nacionalidade")),
	rg: z
		.string({ error: defaultError("RG") })
		.nonempty(defaultEmptyError("RG"))
		.min(12, defaultError("RG")),
	cpf: z
		.string({ error: defaultError("CPF") })
		.nonempty(defaultEmptyError("CPF"))
		.min(14, defaultError("CPF")),
	...
	postalCode: z
		.string({ error: defaultError("Código postal") })
		.nonempty(defaultEmptyError("Código postal"))
		.min(9, defaultError("Código postal")),
